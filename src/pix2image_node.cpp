...
void processCallback(const sensor_msgs::ImageConstPtr& msg, 
                     const std::string& s, ros::NodeHandle &node_handle, const bool save=false){
  ...
  if (s == "stokes"){
    ...
    std::vector<cv::Mat> output_img = POLPro::compute_stokes(img, false);
...
int main( int argc, char** argv )
{
  ...
  if (argc <= 1) {
    std::cout <<" Usage: display_image Option (mat, stokes, polar) (save/None)" 
	      << std::endl;
    return -1;
  }
  std::string s = argv[1]; 
