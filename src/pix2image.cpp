...
namespace POLPro
{
  ...
  std::vector<cv::Mat> polar_stokes_preprocessing(std::vector<cv::Mat> img, const bool as_stokes=true){
  ...
  void imshow(std::vector<cv::Mat> img, const bool as_hsv=false,
	...
	img[0] = img[0] * (cv::pow(2, 16) -1);
	//angle is between 0 and 180 to show it in the CV_16U we just applify it by the maxVal/2
	// img[1] = img[1] + (cv::pow(2, 10) -1); // original computation
	img[1] = (img[1] / 180.0) * (cv::pow(2, 16) -1);  // added by Olivier
	img[2] = img[2] / 2;
      }
      // Convert to uint16
  ...
  // Only used to save one the last image
